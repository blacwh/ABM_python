...
class Particle:
    ...
    def advance(self, dt):#move function
        ...
        if (self.time - self.last_change_time) > self.wander_step_duration:
            ...
            self.vx = int(np.random.randn() * 5)/100
            self.vy = int(np.random.randn() * 5)/100
            self.last_change_time = self.time

        self.r += self.v * dt

        # self.vx = np.random.randn(1)/10
        # self.vy = np.random.randn(1)/10
        # self.r += self.v * dt
...
class Simulation:
    ...
    def __init__(self, n, infectn, radius, iterations):
    ...
    def setup_animation(self):
        ...
        self.ax[1].set_xlim(0,self.iterations/25)
    ...
    def do_animation(self, save=False, filename='abm1.mp4'):
        ...
        anim = animation.FuncAnimation(self.fig, self.animate,
                init_func=self.init, frames=self.iterations, interval=10, blit=False)
...
if __name__ == '__main__':
    ...
    infectn = 2
    iterations = 2000
    sim = Simulation(nparticles, infectn, radius, iterations)
